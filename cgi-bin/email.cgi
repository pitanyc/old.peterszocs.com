...
my $emailConfPath = "/etc/apache2/email.conf";
...
#newlines allowed
my $content = &clean($query->param('message'));

#Note: subject is not mandatory, but you can easily change that
if (($name eq "") || ($email eq "") || ($subject eq "") || ($content eq "") || ($recipient eq ""))
{
...
open(OUT, "|$sendmail -F \"Z Feedback\" -t");
